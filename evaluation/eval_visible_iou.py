...
def eval_dataset_visible_IoU(dataset, show=False):
    bar = tqdm(dataset, total=len(dataset), ncols=200)
    iou2ds = []
    for data in bar:
...
if __name__ == '__main__':
    ...
    # iou2d_d = execute_eval_dataset_visible_IoU(root_dir='/data/cylin/zzs/Datasets/mp3d',
    #                                            dataset=MP3DDataset,
    #                                            modes=['train', 'test', 'val'])

    # iou2d_d = execute_eval_dataset_visible_IoU(root_dir='/data/cylin/zzs/Datasets/pano_s2d3d',
    #                                            dataset=PanoS2D3DDataset,
    #                                            modes=['train', 'test', 'val'])

    iou2d_d = execute_eval_dataset_visible_IoU(root_dir='/data/cylin/zzs/Datasets/zind',
                                               dataset=ZindDataset,
                                               modes=['train', 'test', 'val'])
